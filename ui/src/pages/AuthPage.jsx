...
import { Zap, UserPlus, LogIn, Play, CheckCircle, Loader, Trash2, AlertTriangle } from 'lucide-react'
      ...
      <div className="bg-surface border border-border rounded-2xl p-8 w-80 shadow-2xl">
        ...
        <div className="space-y-3">
          {DEMO_STEPS.map((label, i) => {
            const done = steps.includes(label) && label !== steps[steps.length - 1]
            ...
            return (
              <div key={label} className="flex items-center gap-3">
                {done ? (
                  ...
                  <Loader size={15} className="text-accent shrink-0 animate-spin" />
                ) : (
                  <div className="w-[15px] h-[15px] rounded-full border border-border shrink-0" />
                )}
                <span className={`text-sm ${done ? 'text-muted line-through' : active ? 'text-text font-medium' : 'text-muted/40'}`}>
                  {label}
                </span>
              </div>
            )
...
export function AuthPage() {
  ...
  const { users, addUser, setCurrentUser, removeUser, clearUsers, toast } = useStore()
  ...
  useEffect(() => {
    ...
    Promise.all(
      users.map(u =>
        api.getUser(u.id).catch(() => { stale++; return null })
      )
    ).then(results => {
      ...
      <div className="min-h-screen flex items-center justify-center p-4">
        <div className="w-full max-w-md space-y-4">
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-3 py-4 bg-accent hover:bg-accent-hover disabled:opacity-50 text-white font-semibold rounded-2xl transition-colors shadow-lg shadow-accent/20 group"
          >
            ...
            Run Demo — create 4 users, posts, likes & comments
          </button>
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      onKeyDown={e => { if (e.key === 'Enter' && !e.shiftKey) handleSignup() }}
                  ...
                  {users.length > 0 && (
                    <p className="text-xs text-center text-muted">
                      Have an account?{' '}
                      <button onClick={() => setTab('login')} className="text-accent hover:underline">Log in</button>
                    </p>
                  )}
                </div>
              )}
                    ...
                    <div className="flex items-start gap-2.5 p-3 bg-yellow-950/40 border border-yellow-800/40 rounded-xl">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-xs text-yellow-200">
                          {staleCount} account{staleCount > 1 ? 's' : ''} could not be found in the backend — likely the services were restarted. They have been removed.
                        </p>
                      </div>
                    </div>
                  )}
                  {validating && (
                    <p className="text-xs text-muted text-center">Checking accounts…</p>
                  )}
                  {users.length > 0 ? (
                    <div className="space-y-2">
                      {users.map(u => (
                        ...
                        >
                          ...
                          <LogIn size={14} className="text-muted group-hover:text-accent transition-colors shrink-0" />
                        </button>
                      ))}
                    </div>
                  ) : (
                    <div className="py-6 text-center">
                      ...
                      <button onClick={() => setTab('signup')} className="text-xs text-accent hover:underline mt-1">
                  ...
                  <div className="pt-1">
                    ...
                    {showAdvanced && (
                      <div className="mt-3 space-y-3">
                        <div className="space-y-2">
                          <p className="text-xs text-muted/70">Look up an account by its system ID</p>
                        ...
                        {users.length > 0 && (
                          <button
                            onClick={() => { clearUsers(); setStaleCount(0) }}
